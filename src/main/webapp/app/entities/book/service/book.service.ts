...
export class BookService {
  ...
  protected convertDateFromClient(book: IBook): IBook {
    return Object.assign({}, book, {
      bookdate: book.bookdate?.isValid() ? book.bookdate.format(DATE_FORMAT) : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.bookdate = res.body.bookdate ? dayjs(res.body.bookdate) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((book: IBook) => {
        book.bookdate = book.bookdate ? dayjs(book.bookdate) : undefined;
      });
    }
    return res;
  }
